...
class WxPayDataBase
{
     ...
     * @param string $value
     **/
    public function SetSign($is_app)
    {
        $sign = $this->MakeSign($is_app);
     ...
     */
    public function MakeSign($is_app=1)
    {
        //签名步骤一：按字典序排序参数
        ksort($this->values);
...
//        if(isset($this->values['trade_type']) && $this->values['trade_type'] == 'JSAPI'){
//            $is_app = 0;
//        }
        if($this->values['appid'] == config('xiaochengxu.app_id')){
            $is_app = 0;
        }
        if($is_app){
            $string = $string . "&key=" . config('weixin.api_key');
        }else{
            $string = $string . "&key=" . config('xiaochengxu.api_key');
        }
        //签名步骤三：MD5加密
        $string = md5($string);
        //签名步骤四：所有字符转为大写
