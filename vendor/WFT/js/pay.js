            ...
            $.post('request.php',param,function(res){
                ...
                if(res.status === 500){
                    ...
                    if(curPage === 'orderInfo'){ 
                        if (res.service == 'unified.trade.pay') {
    						$('body').popUpWin({
                                content: '得到token_id='+res.token_id+'与services='+res.services
                            }); 
                        } else if (res.service == 'pay.weixin.raw.app') {
                            $('body').popUpWin({
                                content: '得到:<br>pay_info='+res.pay_info+'<br>out_trade_no='+res.out_trade_no+'<br>transaction_id='+res.transaction_id
                            });
                        }
                    }else{
                        $('body').popUpWin({
